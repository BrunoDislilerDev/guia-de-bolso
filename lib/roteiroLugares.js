...
/**
 * Pontua relevância de um lugar para os interesses informados.
...
export function selecionarLugaresParaRoteiro(lugaresRaw, interesses, limite = 24) {
  ...
  if (relevantes.length >= 8) {
    return relevantes.map(({ score: _score, ehParceiro: _p, ...rest }) => rest);
  }

  return comScore.slice(0, limite).map(({ score: _score, ehParceiro: _p, ...rest }) => rest);
}
