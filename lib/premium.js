 ...
 * Aceita formato diário (`YYYY-MM-DD`) ou legado mensal (`YYYY-MM`).
...
export function isSameUsageDay(usoIaMes, day = getUsageDayKey()) {
  if (!usoIaMes) return false;
  if (usoIaMes === day) return true;
  if (/^\d{4}-\d{2}$/.test(usoIaMes)) {
    return day.startsWith(`${usoIaMes}-`);
  }
  return false;
}
...
export function normalizeUsageFromPerfil(perfil) {
  ...
  const buscas = sameDay ? Number(perfil?.buscas_ia) || 0 : 0;
  const roteiros = sameDay ? Number(perfil?.roteiros_ia) || 0 : 0;
