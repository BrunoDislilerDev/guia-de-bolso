...
function serializeRotaForLanding(rota) {
  ...
  return {
    id: rota.id,
    titulo: rota.titulo,
    descricao: rota.descricao?.slice(0, 100) ?? "",
    capa: capa || null,
  };
}
...
export async function fetchLandingPageData() {
  ...
  const [lugaresRes, rotasRes] = await Promise.all([
    queryLugaresAtivos(supabase, { limit: 80, select: LANDING_LUGAR_SELECT }),
    supabase
      .from("rotas")
      .select("id, titulo, descricao, fotos, foto_capa, imagem_capa, ativa")
      .eq("ativa", true)
  ...
  const parceirosCount = lugares.filter((l) => isParceiro(l)).length;
 ...
 * @property {{ totalLugares: number, categoriasComLugares: number, parceirosCount: number, rotasCount: number }} stats
 ...
 * @property {Array<{ id: string, titulo: string, descricao: string, capa: string|null }>} rotas
