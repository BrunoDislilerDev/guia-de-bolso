...
async function incrementBuscaFallback(supabase, userId, user) {
  ...
  const aligned = shouldAlignUsageToDay(perfil, day)
    ...
    : perfil;
  const { buscas: used, roteiros: roteirosAtual } = getEffectiveUsageCounters(
    aligned,
    day
  );
  ...
  const nextPerfil = buildOptimisticPerfil(aligned, day, {
    buscas_ia: used + 1,
    roteiros_ia: roteirosAtual,
  });

  await persistUsageCounters(supabase, userId, user, aligned, day, {
    buscas_ia: used + 1,
    roteiros_ia: roteirosAtual,
  });
...
async function incrementRoteiroFallback(supabase, userId, user) {
  ...
  const aligned = shouldAlignUsageToDay(perfil, day)
    ? { ...perfil, uso_ia_mes: day, buscas_ia: 0, roteiros_ia: 0 }
    : perfil;
  const { buscas: buscasAtual, roteiros: used } = getEffectiveUsageCounters(
    aligned,
    day
  );
