...
/**
 * Registra uma busca com IA bem-sucedida (incrementa contador diário).
 * Chamar apenas após a operação concluir com sucesso.
 * @param {string|null|undefined} userId
 * @param {{ user?: import('@supabase/supabase-js').User|null }} [options]
 * @returns {Promise<ServerAccessResult>}
 */
export async function recordBuscaIaUsage(userId, { user = null } = {}) {
  return checkBuscaAccess(userId, { increment: true, user });
}

/**
 * Registra um roteiro com IA gerado com sucesso (incrementa contador diário).
 * Chamar apenas após a operação concluir com sucesso.
 * @param {string|null|undefined} userId
 * @param {{ user?: import('@supabase/supabase-js').User|null }} [options]
 * @returns {Promise<ServerAccessResult>}
 */
export async function recordRoteiroIaUsage(userId, { user = null } = {}) {
