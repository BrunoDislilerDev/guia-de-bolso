...
import { fetchDestaquesAdmin, getDestaqueStatus, hojeISO } from "@/lib/destaques";
...
export async function fetchAdminAlertas(supabase) {
  const hoje = hojeISO();
  const limiteExpirando = addDaysISO(hoje, DIAS_ALERTA_DESTAQUE);
  ...
  for (const destaque of destaques) {
    const nome = destaque.lugares?.nome || "Local";
    ...
    if (
      ...
      const dias = diasAteFim(fim, hoje);
      alertas.push({
        tipo: ALERTA_TIPO.DESTAQUE_EXPIRANDO,
        prioridade: ALERTA_PRIORIDADE.ALTA,
        dedupeKey: `${ALERTA_TIPO.DESTAQUE_EXPIRANDO}:${destaque.id}`,
        titulo: `Destaque de ${nome} expira em ${dias} ${plural(dias, "dia", "dias")}`,
        mensagem: `Vigente até ${fim}. Renove em Destaques se necessário.`,
        href: "/admin/destaques?status=expirando",
        createdAt: destaque.data_fim || new Date().toISOString(),
        icon: ALERTA_ICONE[ALERTA_TIPO.DESTAQUE_EXPIRANDO],
      });
    }

    if (destaque.ativo && status === "expirado") {
      alertas.push({
        tipo: ALERTA_TIPO.DESTAQUE_EXPIRADO_ATIVO,
        prioridade: ALERTA_PRIORIDADE.ALTA,
        dedupeKey: `${ALERTA_TIPO.DESTAQUE_EXPIRADO_ATIVO}:${destaque.id}`,
        titulo: `Destaque de ${nome} expirou — ainda ativo`,
        mensagem: "Desative ou estenda o período no painel de destaques.",
        href: "/admin/destaques?status=expirado",
        createdAt: destaque.data_fim || new Date().toISOString(),
        icon: ALERTA_ICONE[ALERTA_TIPO.DESTAQUE_EXPIRADO_ATIVO],
      });
    }
  }
