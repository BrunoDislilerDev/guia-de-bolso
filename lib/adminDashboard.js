 ...
 * @returns {string}
 */
export function addDaysISO(hoje, dias) {
 ...
 */
export async function countDestaquesExpirando7d(supabase) {
  const hoje = hojeISO();
  const limite = addDaysISO(hoje, 7);
  const destaques = await fetchDestaquesAdmin(supabase);
...
export function buildResumoOperacional(counts) {
  ...
  if (counts.destaquesExpirando > 0) {
    parts.push(
      `${counts.destaquesExpirando} destaque${counts.destaquesExpirando !== 1 ? "s" : ""} expirando`
    );
  }
