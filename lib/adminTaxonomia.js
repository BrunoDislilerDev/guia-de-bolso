...
export async function fetchTagsAdmin(supabase) {
  ...
  if (isSubcategoriasColumnError(error) || isAplicaEmRotasColumnError(error)) {
...
export async function createTagAdmin(supabase, payload) {
  ...
  if (!row.subcategorias.length && !row.categorias.length && !row.aplica_em_rotas) {
    return {
      data: null,
      error:
        "Selecione ao menos uma subcategoria, uma categoria ou marque “Aplica em rotas”.",
    };
  }

  let { data, error } = await supabase.from("tags").insert(row).select().single();

  if (error && (isSubcategoriasColumnError(error) || isAplicaEmRotasColumnError(error))) {
