...
const CLIMA_CACHE_TTL_MS = 10 * 60 * 1000;

/** Categorias de lugar que exibem clima/mar no detalhe. */
export const CATEGORIAS_COM_CLIMA = ["Natureza", "Aventura"];
...
/**
 * Indica se o lugar deve exibir widget de clima (categoria ao ar livre + coordenadas).
 * @param {{ categoria?: string }} [lugar]
 * @param {{ latitude?: number|string, longitude?: number|string }} [localizacao]
 * @returns {boolean}
 */
export function lugarExibeClima(lugar, localizacao) {
  ...
  return CATEGORIAS_COM_CLIMA.includes(lugar?.categoria);
}

/**
 * Busca clima com cache em memória por coordenadas (~10 min).
 * @param {number} latitude
 * @param {number} longitude
 * @returns {Promise<Object>}
 */
export async function fetchClimaApisCached(latitude, longitude) {
  const key = `${Number(latitude).toFixed(3)},${Number(longitude).toFixed(3)}`;
  const cached = climaCache.get(key);

  if (cached && Date.now() - cached.at < CLIMA_CACHE_TTL_MS) {
    return cached.data;
  }

  const data = await fetchClimaApis(latitude, longitude);
  climaCache.set(key, { data, at: Date.now() });
  return data;
}
